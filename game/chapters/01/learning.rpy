...
label .math_lesson:
    ...
    "Перед тем как потерять сознание, я понял, что это была Светлана Владимировна"
    scene black with diss
...
label .dream:
    ...
    "Контрольную..."
    "СТОП"
    "Это же не аудитория, а квартира Светланы Владимировны!"
    ...
    menu:
        "Привлечь внимание Светланы Владимировны?"
        "Да!":
            $vmeshalsya = True
...
label .interfere_in_conversation:
    ...
    i "Светлана Владимировна, извините, вы сейчас пишете нам задания на контрольную?"
    math_teacher "Да"
    "Она ответила и вернулась к своим записям"
    i "Светлана Владимировна, вы не перенесете проверочную на следующую пару?"
    ...
    menu:
        ...
        "Напомнить о празднике":    
            $condition = True
            i "Следующая пара как раз выпадает на день учителя, вы же не будете в свой праздник мучать студентов, верно?"
            i "Соберетесь с другими преподавателями, попьете чаю, обсудите студентов..."
            i "Зачем вам эта контрольная?"
            math_teacher "А ведь точно!"
            math_teacher "Я совсем забыла, что на носу день учителя..."
            math_teacher "Нужно по традиции купить всем маленькие подарки"
            math_teacher "Но у меня нет времени на поход по магазинам..."
            math_teacher "Придти без подарков никак нельзя, если не куплю их - лучше вообще не идти..."
            math_teacher "Ладно, я перенесу контрольную и вместо подготовки заданий выберу подарки"
            "Ура, получилось!"
            scene black with diss
            "Я погрузился во мрак и снова потерял сознание"
    return
...
label .not_interfere_in_conversation:
    ...
    "Несколько минут спустя я заметил, что Светлана Владимировна отложила ручку и встала из-за стола"
    ...
    menu:
        ...
        "Нет":
            ...
            "Сейчас Светлана Владимировна пройдет здесь и увидит меня!"
...
label object_transfer_in_dream:
    ...
    scene bg white_table_with_answers with diss
    "Это же ответы!"
...
label math_test:
    ...
    #Не вмешался, достал ответы
        ...
        "Но я запомнил все те ответы, которые были во сне"
        "Надеюсь, я не зря решил учить ответы вместо того, чтобы разобраться в теме..."
        call neo_win
    ...
    #Не получилось перенести кр
        ...
        math_teacher "[name] - 2, хотя, будь моя воля, я бы поставила кол"
